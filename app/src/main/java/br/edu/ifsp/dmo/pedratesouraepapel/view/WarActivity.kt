package br.edu.ifsp.pedratesouraepapel.view
...
import br.edu.ifsp.jukenpo.R
import br.edu.ifsp.jukenpo.databinding.ActivityWarBinding
import br.edu.ifsp.jukenpo.model.Player
import br.edu.ifsp.jukenpo.model.War
import br.edu.ifsp.jukenpo.model.Weapon
class WarActivity : AppCompatActivity() {
    ...
    private var playAgainstBot: Boolean = false
    ...
    private fun openBundle() {
        ...
        if (extras != null) {
            ...
            playAgainstBot = extras.getBoolean(Constants.KEY_PLAY_AGAINST_BOT)
    ...
    private fun battle() {
        val winner: Player?
        if (weaponPlayer1 != null && (weaponPlayer2 != null || playAgainstBot)) {
            if (playAgainstBot && weaponPlayer2 == null) {
                weaponPlayer2 = getRandomWeapon()
            }
            winner = war.toBattle(weaponPlayer1!!, weaponPlayer2!!)
            if (winner != null) {
                Toast.makeText(this, "${getString(R.string.winner)} ${winner.name}", Toast.LENGTH_LONG).show()
            } else {
    ...
    private fun getRandomWeapon(): Weapon {
        val weapons = arrayOf(Rock, Paper, Scissors)
    ...
    private fun proclaimWinner() {
        val str = "${war.getWinner().name}$
        {getString(R.string.won_the_march)}"
    ...
    private fun updateUI() {
        ...
        updateScoreBoard()
        binding.buttonWeapon1.text = "${war.opponent1.name} $
        {getString(R.string.gum_selection)}"
        binding.buttonWeapon2.text = "${war.opponent2.name} $
        {getString(R.string.gum_selection)}"
    }
}
